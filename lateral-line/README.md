...
**At-least-once delivery.** If Axol is down, items stay in KV and retry next sample. If the script crashes between forward and ack, you may see one duplicate bubble — acceptable for MVP.
...
- **Liveness probe.** Before hitting the cloud each sample, the script does a bare TCP connect to `127.0.0.1:47329`; if Axol isn't listening it bails cleanly, leaving items queued for the next run.
...
## Gills (pull-style integrations)

For services that don't webhook out to you — GitHub personal-access-token notifications are the canonical example — the `gills/` directory has small companion scripts that pull on a `launchd` schedule and POST straight into Axol's loopback receiver. Named after the feathery external gills axolotls use to sample the water around them; same rhythmic-sampling motion a polling loop makes. Credentials live in macOS Keychain; no neuromast involvement. See [`gills/README.md`](./gills/README.md) for the full list + how to write a new one.
